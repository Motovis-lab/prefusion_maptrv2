...
from mmengine import ConfigDict, build_from_cfg, deprecated_api_warning, to_2tuple, MODELS
...
@MODELS.register_module()
class CmtStreamTransformer(CmtTransformer):
    def __init__(self, encoder=None, decoder=None, init_cfg=None, cross=False):
        super().__init__(encoder, decoder, init_cfg, cross)

    def forward(self, x, x_img, target, query_embed, bev_pos_embed, rv_pos_embed, attn_masks=None, reg_branch=None,
                **kwargs):
        """Forward function for `Transformer`.
        Args:
            x (Tensor): Input query with shape [bs, c, h, w] where
        ...
        target_query_pos_emb = query_embed.transpose(0, 1)  # [num_query, dim] -> [num_query, bs, dim]
        target = target.transpose(0, 1)

        memory, memory_pos_embed = torch.cat([bev_memory, rv_memory], dim=0), torch.cat([bev_pos_embed, rv_pos_embed],
                                                                                        dim=0)
        mask = memory.new_zeros(bs, memory.shape[0])  # [bs, n, h, w] -> [bs, n*h*w]
        # out_dec: [num_layers, num_query, bs, dim]

        # 需要的内容
        # target_query, target_query_pos_emb
        # memory, memory_pos_emb
        # temp_query, temp_query_pos_emb  # 这俩在kwargs中，并没有做修改；只有上面四个

        out_dec = self.decoder(
            query=target,  # tgt
            key=memory,  # memory
            value=memory,
            key_pos=memory_pos_embed,  # memory
            query_pos=target_query_pos_emb,  # tgt pos
            key_padding_mask=mask,
            attn_masks=[attn_masks, None],
            reg_branch=reg_branch,
            **kwargs
