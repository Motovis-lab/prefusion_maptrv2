...
# tensor dimension corresponed physical range, ([Z_min, Z_max], [X_min, X_max], [Y_min, Y_max])
...
transformables=dict(
    ...
    pin_3d_no_heading=dict(
        type='Bbox3D', 
        loader=dict(type='AdvancedBbox3DLoader', 
                    class_mapping=mapping_wheel_stopper['class_mapping']),
        tensor_smith=dict(type='PlanarBbox3D_pin', voxel_shape=voxel_shape, voxel_range=voxel_range)),
...
train_dataset = dict(
    ...
    info_path='/mnt/disk1/Motovis_Dataset/mv-4d-annotation/mv4d_local/fb_train_3dgs_20251107_clean.pkl',
    model_feeder=dict(
    ...
    group_sampler=dict(type="IndexGroupSampler",
                    ...
                    #    indices_path="/mnt/disk1/Motovis_Dataset/mv-4d-annotation/mv4d_local/train_indice_20251107_all_parking_balanced_IPMremove640x2_old20cm_new10cm_3dgs10cm_4dErrorDumped_3dgsErrorDumped.txt"),
...
val_dataset = dict(
    ...
    name="demo_parking",
    data_root='/mnt/disk1/Motovis_Dataset/mv-4d-annotation/mv4d_local',
    info_path='/mnt/disk1/Motovis_Dataset/mv-4d-annotation/mv4d_local/fb_train_20251107_all_borui_3dgs_IPM_clean.pkl',
...
# backbone = dict(type='ResNet18LiteFPN',c2_channels=64,c3_channels=128,c4_channels=194, c5_channels=256,out_channels=camera_feat_channels)
backbone = dict(type='VoVNetLiteFPN', out_channels=camera_feat_channels, relu6=relu6)
# spatial_transform
spatial_transform = dict(
    type='FastRaySpatialTransformRelu6',
    voxel_shape=voxel_shape,
...
## voxel encoder
voxel_encoder = dict(
    type='VoxelEncoderLiteFPN', 
    in_channels=96, 
    mid_channels_list=[96, 96, 96],
    out_channels=96,
    repeats=[4, 4, 5],
    relu6=relu6)
...
heads = dict(
    ...
    polyline_3d=dict(type='PlanarHeadSimple',
                     ...
                     reg_channels=7 + 7,
                     relu6=relu6),
    parkingslot_3d=dict(type='ParkingSlotHead_ven',
                        in_channels=96,
                        mid_channels=64,
                        cen_seg_channels=5,
                        reg_channels=15,
...
polyline_3d_weight_scheme = dict(
    seg=dict(loss_weight=0.0,
             iou_loss_weight=5,
             dual_focal_loss_weight=100),
    reg=dict(loss_weight=0.0,
             partition_weights={
                "dist": {"weight": 2, "slice": (0, 1)},
                "vec": {"weight": 2, "slice": (1, 3)},
                "dir": {"weight": 20, "slice": (3, 6)},
...
parkingslot_3d_weight_scheme = dict(
    ...
    # pts=dict(loss_weight=40,
    #          loss_weight_seg=30.0,
    #          loss_weight_reg=150.0),     

    # pin=dict(loss_weight=200,
    #          loss_weight_seg=30.0,
    #          loss_weight_reg=150.0),     

    pts=dict(loss_weight=4,
             loss_weight_seg=30.0,
             loss_weight_reg=150.0),     

    pin=dict(loss_weight=16,
             loss_weight_seg=30.0,
             loss_weight_reg=150.0),   


    # pts=dict(loss_weight=10,
    #          loss_weight_seg=60.0,
    #          loss_weight_reg=0.0),     

    # pin=dict(loss_weight=20,
    #          loss_weight_seg=60.0,
    #          loss_weight_reg=0.0),  
...
model = dict(
    type='ParkingFastRayPlanarSingleFrameModelAPA_Parking_IPM',
    backbone=backbone,
...
## log_processor
log_processor = dict(type='GroupAwareLogProcessor', window_size=1000)
# default_hooks = dict(logger=dict(type='LoggerHook', interval=100),timer=dict(type='GroupIterTimerHook'))
default_hooks = dict(
    logger=dict(type='LoggerHook', interval=1000),
    timer=dict(type='GroupIterTimerHook'),
    checkpoint=dict(type='ExperimentWiseCheckpointHook', interval=1, by_epoch=True))


## runner loop configs
train_cfg = dict(type="GroupBatchTrainLoop", max_epochs=15, val_interval=-1)
...
## scheduler configs
# param_scheduler = dict(type='MultiStepLR', milestones=[6, 12, 18, 24], gamma=0.1)

# param_scheduler = dict(type='MultiStepLR', milestones=[2, 5, 9, 12], gamma=0.1)

param_scheduler = dict(type='MultiStepLR', milestones=[3, 6, 9, 12], gamma=0.1)

# param_scheduler = dict(type='MultiStepLR', milestones=[2, 4], gamma=0.1)
# param_scheduler = dict(type='MultiStepLR', milestones=[3, 6, 9], gamma=0.1)
# param_scheduler = dict(type='MultiStepLR', milestones=[6, 12, 18, 36])
# param_scheduler = [
#     # 1️⃣ warm-up 阶段（前 500 iters）
#     dict(
#         type='LinearLR',
#         start_factor=0.1,   # 起始是 base_lr × 0.001
#         by_epoch=False,
#         begin=0,
#         end=500               # warm-up 到第 500 个 iteration 结束
#     ),
...
work_dir = "./work_dirs/fastray_planar_single_frame_park_apa_1107_debug"
# load_from = "./work_dirs/fastray_planar_single_frame_park_apa_0820/20250821_000400/epoch_5.pth"
# load_from = "./work_dirs/pretrained/scene_104_epoch_74.pth"
# load_from = "./work_dirs/fastray_planar_single_frame_park_apa_1028/20251027_205847/epoch_7.pth"
# load_from = "./work_dirs/fastray_planar_single_frame_park_apa_0618/20250619_235048/epoch_11.pth"
# load_from = "./work_dirs/fastray_planar_single_frame_park_apa_0618/20250619_235048/epoch_11.pth"
