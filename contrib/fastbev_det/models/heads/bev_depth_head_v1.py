        ...
        for data in batch_data:
            ...
            for task_name in self.tasks:
                for box_num_id, boxes in enumerate(data['transformables'][task_name['label_type']].elements):
                    ...
                    tmp_box = [*boxes['translation'][:, 0].tolist(), *boxes['size'], Rotation.from_matrix(boxes['rotation']).as_euler("xyz", degrees=False).tolist()[-1], *boxes['velocity'][:, 0].tolist()[:2]]
                    if tmp_box[3] < tmp_box[4]:
                        ...
                        bboxes_3d.append([*boxes['translation'][:, 0].tolist(), *boxes['size'], Rotation.from_matrix(R_nus @ boxes['rotation']).as_euler("xyz", degrees=False).tolist()[-1], *boxes['velocity'][:, 0].tolist()[:2], front_center_z, back_center_z])
                    else:
                        box_R = boxes['rotation'].T
                        corners = get_corners_with_angles(np.array(tmp_box)[None], box_R)[0][:4, :3]
                        front_center_z = corners[:2, 2].mean(0)
                        back_center_z = corners[2:, 2].mean(0)
                        bboxes_3d.append([*boxes['translation'][:, 0].tolist(), *boxes['size'], Rotation.from_matrix(boxes['rotation']).as_euler("xyz", degrees=False).tolist()[-1], *boxes['velocity'][:, 0].tolist()[:2], front_center_z, back_center_z])
