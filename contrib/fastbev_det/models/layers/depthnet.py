...
class DepthNet(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, context_channels,
        ...
        self.depth_channels_fish = int(
            (d_bound_fish[1] - d_bound_fish[0]) / d_bound_fish[2])
        self.depth_channels_pv = int(
            (d_bound_pv[1] - d_bound_pv[0]) / d_bound_pv[2])
        self.depth_channels_front = int(
            (d_bound_front[1] - d_bound_front[0]) / d_bound_front[2])
        ...
        self.depth_fish_conv = nn.Conv2d(mid_channels,
                                         self.depth_channels_fish,
                                         kernel_size=1,
                                         stride=1,
                                         padding=0)
        self.depth_pv_conv = nn.Conv2d(mid_channels,
                                       self.depth_channels_pv,
                                       kernel_size=1,
                                       stride=1,
                                       padding=0)
        self.depth_front_conv = nn.Conv2d(mid_channels,
                                          self.depth_channels_front,
                                          kernel_size=1,
                                          stride=1,
                                          padding=0)
    ...
    def forward(self, x, mats_dict, camera_type):
        mlp_input = torch.cat(
            [
                mats_dict['intrinsic'],
                mats_dict['extrinsic'],
        ...
        supervised_depth = getattr(self, f"depth_{camera_type}_conv")(depth).softmax(dim=1)
