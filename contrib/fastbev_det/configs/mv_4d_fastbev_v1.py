...
model_test_cfg = dict(
    post_center_limit_range=bev_range,
    to_mv_coord = [36, 12, 3],
    grid_size=[240, 120, 1],
    voxel_size=[0.2, 0.2, 3],
)
...
model = dict(
    ...
    head_conf=dict(
        ...
        tasks=[
            dict(label_type='bbox_3d', num_class=4, class_names=['class.vehicle.passenger_car', 'class.traffic_facility.box', 'class.traffic_facility.soft_barrier', 'class.traffic_facility.hard_barrier']),
            dict(label_type='bbox_bev', num_class=3, class_names=['class.road_marker.arrow', 'class.traffic_facility.speed_bump', 'class.parking.wheel_stopper']),
            dict(label_type='square_3d', num_class=1, class_names=['class.parking.indoor_column']),
            ],
        common_heads=dict(reg=(8, 2),
                          ),
        bbox_coder=dict(
...
load_from = "work_dirs/mv_4d_fastbev_v1/epoch_20_ema.pth"
resume=False