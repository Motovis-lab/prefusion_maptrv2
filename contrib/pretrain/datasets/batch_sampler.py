...
__all__ = ['SameSourceBatchSampler']
...
class SameSourceBatchSampler(BatchSampler):
    ...
    def __init__(self,
        ...
        self._same_channel_buckets = [[] for _ in range(2)]

    def __iter__(self) -> Sequence[int]:  # type: ignore
        for idx in self.sampler:
            data_info = self.sampler.dataset.get_data_info(idx)
            if data_info['scene_name'] == "avp":
                bucket_id = 0
            else:
                bucket_id = 1
            bucket = self._same_channel_buckets[bucket_id]
            bucket.append(idx)
        ...
        left_data = self._same_channel_buckets[0] + self._same_channel_buckets[
            1]
        self._same_channel_buckets = [[] for _ in range(2)]
