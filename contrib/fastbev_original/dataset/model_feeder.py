...
from contrib.fastray_planar.voxel_lut import VoxelLookUpTableGenerator
...
__all__ = ["FastBEVModelFeeder"]


@MODEL_FEEDERS.register_module()
class FastBEVModelFeeder(BaseModelFeeder):
    def __init__(self, 
            ...
            for transformable in input_dict['transformables'].values():
                match transformable:
                    ...
                    case Bbox3D():
                        ...
                        match anno_ts:
                            case XyzLwhYawVeloBbox3D() | PlanarBbox3D():
                            ...
                            case Bbox3DBasic():
                                anno_batch_dict[transformable.name]['classes'].append(annotation_tensor['classes'])
                                anno_batch_dict[transformable.name]['xyz_lwh_yaw_vx_vy'].append(annotation_tensor['xyz_lwh_yaw_vx_vy'])
                                anno_batch_dict[transformable.name]['corners'].append(annotation_tensor['corners'])
                            case _:
