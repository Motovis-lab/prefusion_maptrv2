...
from mmdet.models.task_modules import build_anchor_generator, build_assigner, build_bbox_coder, build_sampler
...
from prefusion.registry import MODELS, MMENGINE_MODELS, MMENGINE_TASK_UTILS
...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def __init__(self,
        ...
        self.anchor_generator = MMENGINE_TASK_UTILS.build(anchor_generator)
        # In 3D detection, the anchor stride is connected with anchor size
        self.num_anchors = self.anchor_generator.num_base_anchors
        # build box coder
        self.bbox_coder = MMENGINE_TASK_UTILS.build(bbox_coder)
        self.box_code_size = self.bbox_coder.code_size

        # build loss function
        self.use_sigmoid_cls = loss_cls.get('use_sigmoid', False)
        self.sampling = loss_cls['type'] not in ['mmdet.FocalLoss', 'GHMC']
        if not self.use_sigmoid_cls:
            self.num_classes += 1
        self.loss_cls = MMENGINE_MODELS.build(loss_cls)
        self.loss_bbox = MMENGINE_MODELS.build(loss_bbox)
        self.loss_dir = MMENGINE_MODELS.build(loss_dir)
    ...
    def _init_assigner_sampler(self):
        ...
        if self.sampling:
            self.bbox_sampler = MMENGINE_TASK_UTILS.build(self.train_cfg.sampler)
        else:
            self.bbox_sampler = PseudoSampler()
        if isinstance(self.train_cfg.assigner, dict):
            self.bbox_assigner = MMENGINE_TASK_UTILS.build(self.train_cfg.assigner)
        elif isinstance(self.train_cfg.assigner, list):
            self.bbox_assigner = [
                MMENGINE_TASK_UTILS.build(res) for res in self.train_cfg.assigner
            ]
    ...
    def get_anchors(self, featmap_sizes, num_imgs, device='cuda'):
    ...
    def get_bboxes_single(self,
        ...
        if cfg.get('use_scale_nms', False):
            ...
            mlvl_bboxes_for_nms = xywhr2xyxyr(box_type_3d(
