...
__all__ = [ "FrankenStreamPETRHead" ]

@MODELS.register_module()
class FrankenStreamPETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is FrankenStreamPETRHead):
        ...
        super(FrankenStreamPETRHead, self).__init__(
            num_classes, 
            in_channels, 
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is FrankenStreamPETRHead:
            convert_dict = {
    ...
    def forward(self, img_feats, uv_locations, img_metas, gt_bboxes_3d, gt_labels, topk_indexes=None, **data):
        """_summary_
        ...
        num_tokens = N * H * W
        img_feats = img_feats.permute(0, 1, 3, 4, 2).reshape(B, num_tokens, C)
        img_feats = topk_gather(img_feats, topk_indexes)

        pos_embed, cone = self.position_embeding(data, uv_locations, topk_indexes, img_metas)

        img_feats = self.memory_embed(img_feats) # 做一个MLP，对原始CNN特征进行一波投影

        # spatial_alignment in focal petr
        img_feats = self.spatial_alignment(img_feats, cone) # MLN (对应Focal-PETR中的spatial alignment module，其形式与MLN正好非常相似)；解释：图像特征中可能包含目标的位置信息，MLN允许通过一个仿射变换将这个信息做一定程度的调整
        pos_embed = self.featurized_pe(pos_embed, img_feats) # 初始的pos_embed只有位置信息，这里使用每个uv上的图像特征来re-weight各个通道的值
        ...
        tgt, query_pos, reference_points, temp_img_feats, temp_pos, rec_ego_pose = self.temporal_alignment(query_pos, tgt, reference_points)

        # transformer here is a little different from PETR
        outs_dec, _ = self.transformer(img_feats, tgt, query_pos, pos_embed, attn_mask, temp_img_feats, temp_pos)

        outs_dec = torch.nan_to_num(outs_dec)
        outputs_classes = []
        outputs_coords = []
        for lvl in range(outs_dec.shape[0]):
            reference = inverse_sigmoid(reference_points.clone()) # 从[0,1]转到(-∞,+∞)的logit空间
            assert reference.shape[-1] == 3
            outputs_class = self.cls_branches[lvl](outs_dec[lvl])
            tmp = self.reg_branches[lvl](outs_dec[lvl])

            tmp[..., 0:3] += reference[..., 0:3]
            tmp[..., 0:3] = tmp[..., 0:3].sigmoid() # 从logit空间转回[0, 1]空间
