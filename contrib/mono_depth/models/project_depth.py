...
from contrib.fastbev_det.models.utils import get_unproj_func
import numpy as np
from prefusion.registry import MODELS


__all__ = ['Fish_BackprojectDepth', 'Fish_Project3D']

@MODELS.register_module()
class Fish_BackprojectDepth(BaseModule):
    """Layer to transform a depth image into a point cloud
    """
    def __init__(self, batch_size, height, width, intrinsic, fov=225):
...
class Fish_Project3D(BaseModule):
    def __init__(self, batch_size, height, width, intrinsic, fov=225, init_cfg: dict | torch.List[dict] | None = None):
