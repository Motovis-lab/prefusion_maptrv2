import cv2
import numpy as np
import virtual_camera as vc
import matplotlib.pyplot as plt

import argparse
import yaml

from pathlib import Path
from scipy.spatial.transform import Rotation
from scipy.io import savemat

def load_calpara(file_path):
    with open(file_path, 'r') as f:
        extr_motovis = {}
        intr_motovis = {}
        for line in f:
            line_list = line.strip().split()
            if len(line_list) == 7:
                extr_motovis[line_list[0]] = np.array(line_list[1:], dtype=np.float64)
            if len(line_list) == 9:
                intr_motovis[line_list[0]] = np.array(line_list[1:], dtype=np.float64)
        return extr_motovis, intr_motovis


def get_extrinsic_from_calpara(extr_motovis):
    h, w, z, pitch, roll, yaw = extr_motovis
    R = Rotation.from_euler('xyz', (pitch, roll, yaw), degrees=False).as_matrix()
    t = np.float32([h, w, z])
    return R, t

def get_intrinsic_from_calpara(intr_motovis):
    p0, p1, p2, p3, cx, cy, fx, fy = intr_motovis
    return cx, cy, fx, fy, p0, p1, p2, p3


def get_voxel_points_in_ego(voxel_shape, voxel_range):
    # voxel_shape = (4, 320, 160)
    # voxel_range = [[-2, 2], [36, -12], [12, -12]]
    # output shape: (3, 4*320*160)
    # voxel_points = np.array([zz, xx, yy]).reshape(3, -1)
    Z, X, Y = voxel_shape

    fz = Z / (voxel_range[0][1] - voxel_range[0][0])
    fx = X / (voxel_range[1][1] - voxel_range[1][0])
    fy = Y / (voxel_range[2][1] - voxel_range[2][0])

    cz = - voxel_range[0][0] * fz - 0.5
    cx = - voxel_range[1][0] * fx - 0.5
    cy = - voxel_range[2][0] * fy - 0.5

    vzz, vxx, vyy = np.meshgrid(np.arange(Z), np.arange(X), np.arange(Y), indexing='ij')

    zz = (vzz - cz) / fz
    xx = (vxx - cx) / fx
    yy = (vyy - cy) / fy

    voxel_points = np.array([xx, yy, zz]).reshape(3, -1)

    return voxel_points



def dump_vcam_lut(cameras_real, cameras_virt, output_path):
    output_path = Path(output_path)
    output_path.mkdir(parents=True, exist_ok=True)
    for cam_id in cameras_virt:
        camera_real = cameras_real[cam_id]
        res = camera_real.resolution
        src_img = np.ones((res[1], res[0], 3), dtype=np.uint8)
        camera_virt = cameras_virt[cam_id]
        _, _, uu, vv = vc.render_image(src_img, camera_real, camera_virt, dump_uu_vv=True)
        with open(output_path / f'uu_vv_{cam_id.lower()}.txt', 'w') as f:
            for u in uu:
                f.write(f'{u:.3f} ')
            f.write('\n')
            for v in vv:
                f.write(f'{v:.3f} ')
            f.write('\n')



class VoxelLookUpTableGenerator:
    '''
    Generate LUTS from <x,y,z> to <img_id,u,v,d>.
    >>> LUT = {
        <cam_id>: dict(
            uu=uu, 
            vv=vv, 
            dd=dd, 
            valid_map=valid_map,
            valid_map_sampled=valid_map_sampled,
            norm_density_map=norm_density_map
        )
    }
    '''
    def __init__(self, 
                 voxel_feature_config : dict, 
                 camera_feature_configs : dict):
        '''
        Parameters
        ----------
        voxel_feature_config : dict
            - voxel_shape: (Z, X, Y) in ego system
            - voxel_range: ([zmin, zmax], [xmin, xmax], [ymin, ymax]) in ego system, in meters
            - ego_distance_max: maximum ego distance in meters
            - ego_distance_step: step size of ego distance
        
        camera_feature_configs: dict
            >>> <cam_id>: {
              ray_distance_num_channel: int # number of channels in ray distance
              ray_distance_start: float # start distance of ray in meters
              ray_distance_step: float # step size of ray distance
              feature_downscale: int #downscale of feature map
            }
        '''
        self.voxel_feature_config = voxel_feature_config
        self.camera_feature_configs = camera_feature_configs
        self.voxel_shape = self.voxel_feature_config['voxel_shape']
        self.voxel_range = np.float32(self.voxel_feature_config['voxel_range'])
        # gen voxel_ego_points, in shape of (3, 4*320*160) <3, Z*X*Y>
        self.voxel_points = get_voxel_points_in_ego(self.voxel_shape, self.voxel_range)
    
    def generate(self, camera_images):
        """ Generate LUTS from `<x,y,z>` to `<img_id,u,v,d>`.
        Parameters
        ----------
        camera_images : dict | CameraImageSet
            dict of CameraImage transformables
        seed : int, optional, by default None
            seed for randomness of sampled valid_map

        Return
        ------
        LUT : dict, from <x,y,z> to <img_id,u,v,d>.
            >>> LUT = dict(
                cam_id=dict(
                    uu=uu, 
                    vv=vv, 
                    dd=dd, 
                    valid_map=valid_map,
                    valid_map_sampled=valid_map_sampled,
                    norm_density_map=norm_density_map
                )
            )
        """

        ego_distance_max = self.voxel_feature_config['ego_distance_max']
        ego_distance_step = self.voxel_feature_config['ego_distance_step']
        distances_ego = np.linalg.norm(self.voxel_points, axis=0)
        distance_bins = np.arange(0, ego_distance_max + ego_distance_step, ego_distance_step)

        LUT = {}

        # loop in cameras, gen LUTS from <x,y,z> to <img_id,u,v,d>
        keys = []
        density_maps = []
        
        for key in camera_images:
            keys.append(key)
            camera_image = camera_images[key]
            # gen camera points, in shape of (3, Z*X*Y)
            R_ec, t_ec = camera_image.extrinsic
            camera_points = R_ec.T @ (self.voxel_points - np.array(t_ec)[..., None])
            # camera feature settings
            downscale = self.camera_feature_configs[key]['feature_downscale']
            resolution = np.array(camera_image.resolution) // downscale
            intrinsics = np.array(camera_image.intrinsic)
            intrinsics[:4] = (intrinsics[:4] + [0.5, 0.5, 0, 0]) / downscale - [0.5, 0.5, 0, 0]
            camera = vc.FisheyeCamera(
                resolution,
                camera_image.extrinsic,
                intrinsics,
            )
            uu_float, vv_float = camera.project_points_from_camera_to_image(camera_points) # in shape of Z*X*Y
            # ray distance settings
            ray_distance_start = self.camera_feature_configs[key]['ray_distance_start']
            ray_distance_step = self.camera_feature_configs[key]['ray_distance_step']
            ray_distance_num_channel = self.camera_feature_configs[key]['ray_distance_num_channel']
            distances = np.linalg.norm(camera_points, axis=0) # Z*X*Y
            dd_float = (distances - ray_distance_start) / ray_distance_step
            # rasterize, Z*X*Y, 6*320*160
            uu = np.round(uu_float).astype(int)  
            vv = np.round(vv_float).astype(int)
            dd = np.round(dd_float).astype(int)
            dd[dd < ray_distance_num_channel] = ray_distance_num_channel - 1
            # get valid maps, Z*X*Y, 6*320*160
            valid_map = (uu >= 0) * (uu < resolution[0]) * (vv >= 0) * (vv < resolution[1]) * (camera_points[2] > 0)
            # add uv_mask
            uv_mask = cv2.resize(camera_image.ego_mask, resolution)
            valid_map *= uv_mask[vv * valid_map, uu * valid_map].astype(bool)
            uu_float[~valid_map] = -1
            vv_float[~valid_map] = -1
            dd_float[~valid_map] = -1
            uu[~valid_map] = -1
            vv[~valid_map] = -1
            dd[~valid_map] = -1
            # allocate LUTS
            LUT[key] = dict(
                uu=uu, vv=vv, dd=dd, valid_map=valid_map
            )

            # gen voxel ray density for each camera, in shape of Z*X*Y
            density_map = np.zeros_like(distances_ego)
            for dist_ind in range(len(distance_bins) - 1):
                valid_dist = (distances_ego > distance_bins[dist_ind]) & (distances_ego <= distance_bins[dist_ind + 1])
                valid_u = uu_float[valid_dist * valid_map]
                valid_v = vv_float[valid_dist * valid_map]
                if len(valid_u) > 0 and len(valid_v) > 0:
                    density_map[valid_dist * valid_map] = (valid_u.max() - valid_u.min()) * (valid_v.max() - valid_v.min())
            density_maps.append(density_map)
            
        # normalize density
        density_maps = np.stack(density_maps)
        density_maps_norm = density_maps / (density_maps.sum(axis=0, keepdims=True) + 1e-5)
        for key_ind, key in enumerate(keys):
            LUT[key]['norm_density_map'] = density_maps_norm[key_ind]
        
        return LUT


def render_vcamera_ego_mask(camera_real, camera_virt):
    res_real = camera_real.resolution
    img = np.zeros((res_real[1], res_real[0], 3), dtype=np.uint8)
    _, ego_mask_virt = vc.render_image(img, camera_real, camera_virt)
    camera_virt.ego_mask = ego_mask_virt
    return ego_mask_virt


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('src_path', type=Path)
    args = parser.parse_args()


    ## DUMP VCAMERA LUTS
    calpara_path = args.src_path / 'calpara.txt'
    if calpara_path.exists():
        extr_motovis, intr_motovis = load_calpara(calpara_path)
        cameras_real = {
            'front': vc.FisheyeCamera(resolution=(1920, 1280), extrinsic=get_extrinsic_from_calpara(extr_motovis['Front']), intrinsic=get_intrinsic_from_calpara(intr_motovis['Front'])),
            'left': vc.FisheyeCamera(resolution=(1920, 1280), extrinsic=get_extrinsic_from_calpara(extr_motovis['Left']), intrinsic=get_intrinsic_from_calpara(intr_motovis['Left'])),
            'right': vc.FisheyeCamera(resolution=(1920, 1280), extrinsic=get_extrinsic_from_calpara(extr_motovis['Right']), intrinsic=get_intrinsic_from_calpara(intr_motovis['Right'])),
            'rear': vc.FisheyeCamera(resolution=(1920, 1280), extrinsic=get_extrinsic_from_calpara(extr_motovis['Rear']), intrinsic=get_intrinsic_from_calpara(intr_motovis['Rear']))
        }
        cameras_real['front'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/front_ego_mask.png'), cv2.IMREAD_GRAYSCALE)
        cameras_real['left'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/left_ego_mask.png'), cv2.IMREAD_GRAYSCALE)
        cameras_real['right'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/right_ego_mask.png'), cv2.IMREAD_GRAYSCALE)
        cameras_real['rear'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/rear_ego_mask.png'), cv2.IMREAD_GRAYSCALE)

    yml_file = args.src_path / 'calibration_center.yml'
    if yml_file.exists():
        cfg = yaml.safe_load(open(yml_file, 'r'))
        cameras_real = {
            'front': vc.FisheyeCamera.init_from_motovis_cfg(cfg['rig']['camera8']),
            'left': vc.FisheyeCamera.init_from_motovis_cfg(cfg['rig']['camera5']),
            'right': vc.FisheyeCamera.init_from_motovis_cfg(cfg['rig']['camera11']),
            'rear': vc.FisheyeCamera.init_from_motovis_cfg(cfg['rig']['camera1'])
        }
        cameras_real['front'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/camera8.png'), cv2.IMREAD_GRAYSCALE)
        cameras_real['left'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/camera5.png'), cv2.IMREAD_GRAYSCALE)
        cameras_real['right'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/camera11.png'), cv2.IMREAD_GRAYSCALE)
        cameras_real['rear'].ego_mask = cv2.imread(str(args.src_path / '../ego_masks/camera1.png'), cv2.IMREAD_GRAYSCALE)

    cameras_virt = {
        'front': vc.create_virtual_fisheye_camera((640, 384), (-120, 0, 0), cameras_real['front'].extrinsic[1]),
        'left': vc.create_virtual_fisheye_camera((640, 384), (-135, 0, 90), cameras_real['left'].extrinsic[1]),
        'right': vc.create_virtual_fisheye_camera((640, 384), (-135, 0, -90), cameras_real['right'].extrinsic[1]),
        'rear': vc.create_virtual_fisheye_camera((640, 384), (-120, 0, 180), cameras_real['rear'].extrinsic[1])
    }

    for key in cameras_real:
        ego_mask = render_vcamera_ego_mask(cameras_real[key], cameras_virt[key])
        # plt.imshow(ego_mask); plt.show()


    dump_vcam_lut(cameras_real, cameras_virt, args.src_path)
    print('virtual camera luts dumped!')


    ## DUMP VOXEL LUT

    virtual_cameras = {
        'front': vc.create_virtual_fisheye_camera(
            (640, 384), (-120, 0, -90), 
            [cameras_real['front'].extrinsic[1][1], -cameras_real['front'].extrinsic[1][0], cameras_real['front'].extrinsic[1][2]]
        ),
        'left': vc.create_virtual_fisheye_camera(
            (640, 384), (-135, 0, 0), 
            [cameras_real['left'].extrinsic[1][1], -cameras_real['left'].extrinsic[1][0], cameras_real['left'].extrinsic[1][2]]
        ),
        'right': vc.create_virtual_fisheye_camera(
            (640, 384), (-135, 0, 180), 
            [cameras_real['right'].extrinsic[1][1], -cameras_real['right'].extrinsic[1][0], cameras_real['right'].extrinsic[1][2]]
        ),
        'rear': vc.create_virtual_fisheye_camera(
            (640, 384), (-120, 0, 90), 
            [cameras_real['rear'].extrinsic[1][1], -cameras_real['rear'].extrinsic[1][0], cameras_real['rear'].extrinsic[1][2]]
        )
    }
    virtual_cameras['front'].ego_mask = cameras_virt['front'].ego_mask
    virtual_cameras['left'].ego_mask = cameras_virt['left'].ego_mask
    virtual_cameras['right'].ego_mask = cameras_virt['right'].ego_mask
    virtual_cameras['rear'].ego_mask = cameras_virt['rear'].ego_mask
    # camera and voxel feature configs
    feature_downscale = 4
    default_camera_feature_config = dict(
        ray_distance_num_channel=32,
        ray_distance_start=0.5,
        ray_distance_step=0.5,
        feature_downscale=feature_downscale)

    camera_feature_configs = dict(
        front=default_camera_feature_config,
        left=default_camera_feature_config,
        rear=default_camera_feature_config,
        right=default_camera_feature_config)

    voxel_shape = (8, 160, 120)  # Z, X, Y in ego system
    voxel_range = ([-1, 3], [12, -12], [9, -9])  # tensor dimension corresponed physical range, ([Z_min, Z_max], [X_min, X_max], [Y_min, Y_max])

    voxel_feature_config = dict(
        voxel_shape=voxel_shape, 
        voxel_range=voxel_range,
        ego_distance_max=16,
        ego_distance_step=2)

    voxel_lut_gen = VoxelLookUpTableGenerator(
        voxel_feature_config=voxel_feature_config,
        camera_feature_configs=camera_feature_configs
    )

    voxel_lut = voxel_lut_gen.generate(virtual_cameras)
    savemat(args.src_path / 'voxel_lookups.mat', voxel_lut)
    print('voxel luts dumped!')


    
