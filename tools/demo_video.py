...
def get_cam_img(ind, info, cameras):
    imgs = []
    for cam_id in cameras:
        ...
        for slot in info['slots']:
            slot = np.float32(slot).reshape(-1, 3)[[0, 1, 2, 3, 0]]
...
def get_bev_img(info):
    ...
    for slot in info['slots']:
        slot = np.float32(slot).reshape(-1, 3)
        plt.fill(slot[[0, 1, 2, 3, 0], 1], slot[[0, 1, 2, 3, 0], 0], 'gray', alpha=0.5)
...
if __name__ == '__main__':
    results_root_path = Path('./gts_and_preds_new/')
    all_inds = sorted([str(p.relative_to(results_root_path))[:-5] for p in results_root_path.glob('**/*.json')])

    demo_video = FFMPEG_VideoWriter('prefusion_tda4_apa.mp4', size=(1280, 640), fps=10)
    for ind in tqdm(all_inds):
        json_path = results_root_path / (ind + '.json')
        info_all = json.load(open(json_path))
        img_cat = get_cam_img(ind, info_all['pred'], cameras)
        bev_img_gt = get_bev_img(info_all['gt'])
        bev_img_pred = get_bev_img(info_all['pred'])
        img_final = np.concatenate([bev_img_pred, img_cat, bev_img_gt], axis=1)
