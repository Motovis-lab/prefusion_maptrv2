...
def get_cfg_dictionary_with_task_name(cfg, task_name: str = None):
    ...
    if task_name is None:
        ...
        if task_name == 'parking_slot':
            output_class_mapping['class.parking.parking_slot'] = 'class.parking.parking_slot'
        else:
            if task_name in ["dictionary_polygons", "dictionary_polylines"]:
                class_mapping = {i: i for i in getattr(cfg, task_name).classes}
                attr_mapping = {}
            else:
                class_mapping = getattr(cfg, task_name).class_mapping
                try:
                    attr_mapping = getattr(cfg, task_name).attr_mapping
                except Exception:
                    attr_mapping = {}
            # class_mapping & attr mapping
...
def vanilla_resampling_repicked(gpdf, gpdf_number, frame_keys, sample_number, resampling=True):
    ...
    if sample_number is not None:
        number_target0 = sample_number
    else:
        number_target0 = max_number / 10
    sampled_groups = []
    # first pick
    # for name in [i for i in list(gpdf.columns) if cnt_per_class[i] <= number_target0]:
    #     # output_list += list(gpdf[gpdf[name] > 0].index)
    ...
    #                        range(int(number_target0 - cnt_per_class[name]))]
    for name in [i for i in sorted(list(gpdf.columns), key=lambda i: cnt_per_class[i], reverse=False) if
                 cnt_per_class[i] < number_target0]:  # congshaodekaishilao
        target_number = max(0, number_target0 - cnt_per_class[name])
        if resampling:
            target_number = max(0, target_number - sum(gpdf.loc[list(sampled_groups)][name] > 0))
        if target_number > 0:
            picked_data_list = list(gpdf[gpdf[name] > 0].index)
            random.shuffle(picked_data_list)
            sample_iterator = cycle(picked_data_list)
            sampled_groups += [next(sample_iterator) for _ in
                           range(int(number_target0 - cnt_per_class[name]))]
    output_list = list(sampled_groups)
...
def generate_output_image_list(frame_keys, class_dict_with_attr, data_root, valid_range, sample_number, group_size=1,
    ...
    if sampling_method == "vanilla":
        sampled_groups = vanilla_resampling_repicked(gpdf, gpdf_number, frame_keys, sample_number=sample_number)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_txt', default='/ssd1/MV4D_12V3L/valid_indice_102.txt')
    ...
    parser.add_argument('--save_txt_prefix', default='/ssd1/MV4D_12V3L/train_indice_102_with_ori')
    parser.add_argument('--task_names', nargs='+',
                        default=["mapping_heading_objects",
                                 "mapping_plane_heading_objects",
                                 "mapping_no_heading_objects",
                                 "mapping_square_objects",
                                 "mapping_cylinder_objects",
                                 "mapping_oriented_cylinder_objects",
                                 "dictionary_polygons",
                                 "dictionary_polylines",
                                 "dictionary_parkingslot"
                                 ])
    ...
    # task_names = ["mapping_heading_objects"]
    for task_name in task_names:
        print('start to:', task_name)
        save_txt_path = f"{args.save_txt_prefix}_{task_name}.txt"
        dictionary = get_cfg_dictionary_with_task_name(cfg, task_name)
        sampled_groups = generate_output_image_list(key_list, dictionary, args.data_root, valid_range=[12, 9],
                                                    sample_number=sample_number, group_size=1)
        write_txt(sampled_groups + key_list, save_txt_path)
        print('save_to:', save_txt_path)

    # sync -aVP /ssd1/MV4D_12V3L/train_indice_102_with_ori* wuhan@192.168.3.148:~/MV4D_12V3L/