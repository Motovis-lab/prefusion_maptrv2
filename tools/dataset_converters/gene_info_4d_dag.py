...
def generate_labels_scene_from_4dMapjson(scene_root, Twes):
    ...
    ) = generate_DS4d_from_4dMapJson(op.join(scene_root, "4d_anno_infos/annos.json"), Twes)  # load½øÀ´×ª³ÉDS4D
...
def convert_virtual_camera(src_camear_root, save_img_root, save_mask_root, real_cam_model, v_cam_paramter, calib,
                           W=1280, H=960):
    # ×öÆäËûÊý¾ÝÐÞ¸´£¬²»ÐèÒªÖØÐÂÉú³ÉÍ¼Ïñ£¬ÕâÀïÉèÖÃÎªNone
...
if __name__ == "__main__":
    ...
    all_cameras = [
        "camera1", "camera2", "camera3", "camera4", "camera5", "camera6", "camera7", "camera8", "camera11", "camera12",
        "camera15"
    ]
    fish_cameras = [
    ...
    for scene_name in scene_names:
        ...
        calib_center['rig']['camera8']['extrinsic'] = [
            *((R_nus.T @ np.array(calib_center['rig']['camera8']['extrinsic'][:3]).reshape(3)).tolist()), *(
                Rotation.from_matrix((R_nus.T @ Rotation.from_quat(
                    calib_center['rig']['camera8']['extrinsic'][3:]).as_matrix())).as_quat().tolist())]
        calib_center['rig']['camera5']['extrinsic'] = [
            *((R_nus.T @ np.array(calib_center['rig']['camera5']['extrinsic'][:3]).reshape(3)).tolist()), *(
                Rotation.from_matrix((R_nus.T @ Rotation.from_quat(
                    calib_center['rig']['camera5']['extrinsic'][3:]).as_matrix())).as_quat().tolist())]
        calib_center['rig']['camera1']['extrinsic'] = [
            *((R_nus.T @ np.array(calib_center['rig']['camera1']['extrinsic'][:3]).reshape(3)).tolist()), *(
                Rotation.from_matrix((R_nus.T @ Rotation.from_quat(
                    calib_center['rig']['camera1']['extrinsic'][3:]).as_matrix())).as_quat().tolist())]
        calib_center['rig']['camera11']['extrinsic'] = [
            *((R_nus.T @ np.array(calib_center['rig']['camera11']['extrinsic'][:3]).reshape(3)).tolist()), *(
                Rotation.from_matrix((R_nus.T @ Rotation.from_quat(
                    calib_center['rig']['camera11']['extrinsic'][3:]).as_matrix())).as_quat().tolist())]
        ...
        assert all([i in Twes.keys() for i in timestamps])  # TODO: È·¶¨ÊÇ·ñÄÚº¬Õâ¸öassert
        ...
        for timestamp in track(timestamps, "Process timestamp ... "):
            ...
            sdf_about = {
                ...
                'sdf_2d': f"{scene_name}/sdf/sdf_2d_-15_-15_15_15/{times_id}.tif",  # ÔÝÊ±²»ÓÃ£¬¿ÉÒÔ²»Òª
